...
public sealed class PetEngine
{
    ...
    private double   _prankStageTimer;
    private double   _prankTargetX;
    ...
    /// <summary>Special one-shot prank sequence: animated walk-in followed by a loaf/sit.</summary>
    public void StartPrank()
    {
        var work = SystemParameters.WorkArea;
        _prankActive = true;
        _prankStageTimer = -0.01;
        _prankTargetX = work.Left + work.Width * 0.48;
        _facing = 1;
        _x = work.Left - _w * 0.9;
        SetFeet(work.Bottom);
        _vx = BaseWalkSpeed * 1.15;
        _anim.Play("walk");
    }

    private void UpdatePrank(double dt)
    {
        var work = SystemParameters.WorkArea;
        SetFeet(work.Bottom);
        if (_prankStageTimer < 0)
        {
            _x += _vx * dt;
            _anim.Play("walk");
            if (_x >= _prankTargetX)
        ...
        _prankStageTimer += dt;
        if (_prankStageTimer < 2.0)
        {
            _anim.Play("loaf");
            return;
        }

        // Briefly wander a little, then return to the center and sit.
        if (_prankStageTimer < 5.0)
        {
            _anim.Play("walk");
            _x += Math.Sin(_prankStageTimer * 2.0) * 12 * dt;
            return;
        }
